...
def test_extractor_shapes():
    m = Extractor()
    fe, aux = m(torch.randn(8, 10, 12))
    assert fe.shape == (8, 4 * 64)
    assert aux["wc"].shape == (8, 56)
    assert aux["fbl"].shape == (8, 2 * 64)
    assert aux["fa"].shape == (8, 10)

def test_extractor_backward_and_mcl_constraint():
    m = Extractor()
    fe, _ = m(torch.randn(4, 10, 12))
    fe.sum().backward()
    m.apply_mcl_constraint()
    w = m.mcl.mcl_conv.weight.data.view(m.mcl.mcl_conv.weight.data.size(0), -1)
...
def test_train_extractor_smoke():
    ...
    w = extractor.mcl.mcl_conv.weight.data.view(extractor.mcl.mcl_conv.weight.data.size(0), -1)
